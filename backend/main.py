...
def _serialize_key(key) -> dict:
    return {**key.model_dump(), "pat": _mask(key.pat), **key_mgr.get_key_status(key)}
...
async def login(req: LoginReq):
    if req.password != config_mgr.config.settings.admin_password:
        return _err("密码错误", 401)
    return {"token": config_mgr.config.settings.admin_password}
...
async def export_keys(request: Request):
    if err := _verify_admin(request):
        return err
    pats = [k.pat for k in sorted(config_mgr.config.keys, key=lambda x: x.order)]
