...
async def proxy_messages(
    ...
    if is_stream and payload:
        ...
        return _stream_with_continuation(
            request, payload, keys, key_mgr, log_mgr, time.time(), auto_continue
        )
...
def _stream_with_continuation(
    ...
    async def generate():
        ...
        try:
            ...
            if tool_use_buf:
                logger.info(
                    f"Discarded incomplete tool_use block ({len(tool_use_buf)} chunks)"
                )
                tool_use_buf.clear()
            ...
            if tool_use_buf:
                logger.info(
                    f"Discarded incomplete tool_use on error ({len(tool_use_buf)} chunks)"
                )
                tool_use_buf.clear()
        ...
        if not cap_triggered:
            logger.info(
                f"Stream truncated ({len(acc_text)} chars). Starting auto-continuation..."
            )
        ...
        for cont in range(max_cont):
                ...
                **payload,
                "messages": [
                    *original_messages,
                    ...
                    {
                        "role": "user",
                        "content": "Continue to output the remaining content; there is no need to repeat what has already been output.",
                    },
                ],
            }
            ...
            for ck in cont_keys:
                try:
                    entry = await key_mgr.get_token(ck.pat)
                except Exception:
                    continue
                cont_headers = {
                    "content-type": "application/json",
                ...
                try:
                    cr = await cc.send(
                        cc.build_request(
                            "POST",
                            target,
                            content=json.dumps(cont_payload).encode(),
            ...
            if cont_got_stop:
                if cont_stop_reason == "max_tokens" and cont + 1 < max_cont:
                    logger.info(
                        f"Continuation {cont + 1} also capped ({len(acc_text)} chars), continuing..."
                    )
                    continue
                logger.info(
                    f"Continuation {cont + 1} completed ({len(acc_text)} total chars)"
                )
                yield _close_events(
                    client_block_idx,
                    client_block_open,
            ...
            logger.info(
                f"Continuation {cont + 1}/{max_cont} truncated ({reason}, {len(acc_text)} chars)"
            )
...
def _log_result(
    key, model, start_time, inp, out, ok, key_mgr, log_mgr, status=None, error=""
):
    duration_ms = int((time.time() - start_time) * 1000)
    if ok:
        key_mgr.record_success(key)
...
def _close_events(
    ...
    if block_open:
        parts.append(
            _sse(
                "content_block_stop",
                json.dumps({"type": "content_block_stop", "index": block_idx}),
            )
        )
    parts.append(
        _sse(
